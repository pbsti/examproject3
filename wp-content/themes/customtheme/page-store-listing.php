...
$search = isset($search) ? $search : 'no'; // Default to 'no' if not set
$sellers = isset($sellers) ? $sellers : []; // Default to an empty array
$limit = isset($limit) ? $limit : 10; // Default limit
$offset = isset($offset) ? $offset : 0; // Default offset
$per_row = isset($per_row) ? $per_row : 3; // Default number of sellers per row
$image_size = isset($image_size) ? $image_size : 'thumbnail'; // Default image size
...
<div id="dokan-seller-listing-wrap" class="grid-view">
    <div class="seller-listing-content">
        <?php if ( isset($sellers['users']) && !empty($sellers['users']) ) : ?>
            ...
            <?php
            // Check if 'count' key exists in $sellers
            $user_count = isset($sellers['count']) ? $sellers['count'] : 0; // Default to 0 if not set
